...
-   アップストリームデータベースのbinlog形式は`ROW`に設定されています。そうでない場合は、 [フォーマットエラー](/tidb-cloud/tidb-cloud-dm-precheck-and-troubleshooting.md#error-message-check-whether-mysql-binlog_format-is-row)回避するために、次のように形式を`ROW`に更新してください。
    ...
    -   Amazon Aurora MySQL または RDS for MySQL: [AWSドキュメント](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/USER_WorkingWithDBInstanceParamGroups.html)の手順に従って新しい DB パラメータグループを作成します。新しい DB パラメータグループに`binlog_format=row`パラメータを設定し、新しい DB パラメータグループを使用するようにインスタンスを変更した後、インスタンスを再起動して有効にします。
